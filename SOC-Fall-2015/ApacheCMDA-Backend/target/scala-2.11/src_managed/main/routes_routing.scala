...
// @LINE:79
case controllers_FollowerController_deleteFollower53_route(params) => {
   call(params.fromPath[Long]("userId", None), params.fromPath[Long]("followerId", None)) { (userId, followerId) =>
        controllers_FollowerController_deleteFollower53_invoker.call(play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.FollowerController]).deleteFollower(userId, followerId))
   }
}
